# GitHub Setup Script - Simple Version
...
if (-not (Test-Path ".git")) {
    Write-Host "[INFO] Git repository not initialized" -ForegroundColor Yellow
...
if ($status -and $status -notmatch "fatal") {
    ...
    if ($commitCount -eq "0" -or $commitCount -match "fatal") {
        Write-Host "[INFO] No commits yet. Creating initial commit..." -ForegroundColor Yellow
        git add .
        ...
        Write-Host "[OK] Repository has commits" -ForegroundColor Green
        Write-Host ""
    }
}
...
if ($remote -and $remote -notmatch "fatal") {
    Write-Host "[INFO] GitHub remote already configured:" -ForegroundColor Yellow
...
if ([string]::IsNullOrWhiteSpace($repoUrl)) {
    Write-Host ""
    Write-Host "[INFO] Skipping automatic setup" -ForegroundColor Yellow
    Write-Host ""
...
if ($repoUrl -notmatch "^https://github\.com/") {
    Write-Host "[WARNING] URL should start with https://github.com/" -ForegroundColor Yellow
